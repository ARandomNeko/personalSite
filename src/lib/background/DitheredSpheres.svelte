<script lang="ts">
	...
	function animate() {
		...
		for (const sphere of spheres) {
			...
			const noiseX =
				Math.sin(time * 0.4 + u.noiseOffsetX) * u.noiseScale +
				Math.sin(time * 0.8 + u.noiseOffsetX * 2.0) * (u.noiseScale * 0.3);
			const noiseY =
				Math.cos(time * 0.5 + u.noiseOffsetY) * u.noiseScale +
				Math.cos(time * 0.7 + u.noiseOffsetY * 1.7) * (u.noiseScale * 0.4);
			const noiseZ =
				Math.sin(time * 0.3 + u.noiseOffsetZ) * u.noiseScale +
			...
			sphere.rotation.set(
				...
				time * 0.1 + orbitAngle * 0.1 + noiseY * 0.08,
	...
	function handleResize() {
		...
		spheres.forEach((sphere, index) => {
			...
			sphere.userData.noiseScale =
				Math.min(window.innerWidth, window.innerHeight) * (0.006 + Math.random() * 0.014);
			sphere.userData.orbitRadius =
				Math.min(window.innerWidth, window.innerHeight) * (0.006 + Math.random() * 0.012);
		});
	}
		...
		function addSphereBatch() {
			...
			for (let i = spheresAddedCount; i < batchEnd; i++) {
				...
				const brightness = isDarkMode
					? 0.45 + baseBrightness * 0.5
					: 0.4 + (1.0 - baseBrightness) * 0.3;
